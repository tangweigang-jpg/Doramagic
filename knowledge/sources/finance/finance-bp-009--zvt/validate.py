...
def enforce_validation(result, output_path: str) -> None:
    ...
    try:
        ...
        if "slow=26" not in src or "fast=12" not in src or "n=9" not in src:
            failures.append(
                "FATAL OV-01: MACD params drifted from (fast=12, slow=26, n=9) — SL-08 violation"
            )
    except Exception:
        pass
    ...
    try:
        if hasattr(result, "get"):
            ...
            if ar is not None and abs(float(ar)) > 5.0:
                failures.append(
                    f"FATAL OV-03: |annual_return|={float(ar):.2f} > 500% — likely look-ahead bias"
                )
    except Exception:
        pass
    ...
    try:
        if hasattr(result, "get"):
            ...
            if dd is not None and abs(float(dd)) > 1.0:
                failures.append(
                    f"FATAL OV-05: |max_drawdown|={float(dd):.2f} > 100% — impossible without leverage"
                )
    except Exception:
        pass
