...
| `type=M` | 数学/模型选择——基于数学推导或统计假设的方法选择（如 MACD 参数、EMA adjust=False、等权平均） |
...
**Finance domain term equivalents**:

| Chinese | English | Context |
|---------|---------|---------|
| 涨停 | daily price limit up | A-share market rule |
| 跌停 | daily price limit down | A-share market rule |
| 印花税 | stamp duty | Transaction cost (sell-side) |
| 前视偏差 | look-ahead bias | Backtesting validity |
| 金叉 | golden cross | MA crossover signal |
| 死叉 | death cross | MA crossover signal |
| 换手率 | turnover rate | Liquidity metric |
| 回撤 | drawdown | Risk metric |
| 年化收益率 | annualized return | Performance metric |
| 夏普比率 | Sharpe ratio | Risk-adjusted return |
| 动量 | momentum | Factor/signal type |
| 均线 | moving average | Technical indicator |
| K线 | candlestick / OHLCV bar | Price data format |
| 复权 | adjustment (forward/backward) | Price adjustment |
| 前复权 | forward-adjusted | Price adjustment type |
| 后复权 | backward-adjusted | Price adjustment type |
| 整手 | round lot (100 shares) | A-share trading unit |
| 佣金 | commission | Transaction cost |
| 滑点 | slippage | Execution cost |
| 持仓 | position / holdings | Portfolio state |
...
**输入**：蓝图 `business_decisions` 中 type=B、type=BA、type=M 及其复合类型（M/BA、M/B 等）的条目。
...
- type=M（数学/模型选择）→ 默认 `violation: FATAL`，归入 semantic_locks
- type=M 的 Spec Lock 必须锁定：模型名称/算法名称 + 关键参数值 + 数值方法配置（如 adjust=False、tol=1e-3）
- 框架 API 惯用法、配置最佳实践 → 归入 implementation_hints，`violation: WARN`
- 路径参数使用 `{workspace}` 占位符

**spec_lock_registry（两层结构，内联在晶体中）**：

Spec Lock 分为两层：业务规则层（semantic_locks）跨数据源不变，框架细节层（implementation_hints）按数据源适配。晶体中必须用以下 YAML 结构声明：

```yaml
spec_lock_registry:
  semantic_locks:  # 业务规则层——跨数据源不变
    - id: SL-{N}
      rule: "{跨数据源不变的业务规则}"
      violation: FATAL
      rationale: "{为什么锁——业务含义}"
      verification: "{如何验证规则被遵守}"
      adapter:
        {framework_a}: "{该框架下的等价实现}"
        {framework_b}: "{该框架下的等价实现}"
    # 金融回测必须包含的 semantic_locks 示例：
    # - id: SL-01
    #   rule: "信号必须延迟到下一个 bar 执行（next-bar execution）"
    #   violation: FATAL
    #   rationale: "防止前视偏差"
    #   verification: "确认 buy/sell signal 有 shift(1) 或等价延迟"
    #   adapter:
    #     zvt: "due_timestamp = happen_timestamp + level.to_second()"
    #     pandas: "signal.shift(1)"
    #     baostock: "signal.shift(1)"
    # - id: SL-02
    #   rule: "卖出成本必须包含印花税 0.05%（2023-08-28 起）"
    #   violation: FATAL
    #   verification: "确认 sell_cost >= 0.0005 + commission"
    #   adapter:
    #     zvt: "sell_cost=0.0011"
    #     pandas: "STAMP_TAX = 0.0005; SELL_COST = STAMP_TAX + COMMISSION"
  implementation_hints:  # 框架细节层——按数据源适配
    - id: IH-{N}
      rule: "{框架特定的实现细节}"
      {framework_a}: "{该框架下的写法}"
      {framework_b}: "{该框架下的写法}"
    # 金融回测必须包含的 implementation_hints 示例：
    # - id: IH-01
    #   rule: "标的标识必须能唯一表示 exchange + code"
    #   zvt: "entity_id = '{type}_{exchange}_{code}' (如 stock_sz_000001)"
    #   baostock: "code = '{exchange}.{code}' (如 sz.000001)"
    #   akshare: "code = '{code}' (如 000001)"
```

**提取规则**：
- 每个 type=B、type=RC 的 BD → 提取为 semantic_lock（violation=FATAL）
- 每个 type=M 的 BD → 提取为 semantic_lock（violation=FATAL），adapter 填写主流数据源的等价实现
- 每个框架 API 细节（如字段命名、调用方式）→ 提取为 implementation_hint
- 每个 semantic_lock 必须为所有蓝图 replaceable_points 中列出的主流数据框架提供 adapter 条目
...
- machine_checkable=true 的约束 → 自动纳入 Hard Gate 候选
...
**降级规则**：当 `business_decisions` 或 `known_use_cases` 不存在时，仅从 `replaceable_points` + `stages` 推导。

### 6.5a. 记忆查询设计（memory_queries）

从蓝图中提取宿主 AI 应主动查询用户记忆的条目：
...
Target blueprint: `finance-bp-050` (skorecard credit scoring)
Domain constraint pool: `knowledge/constraints/domains/finance.jsonl` (3655 constraints across 59 blueprints)

Matching scenario: bp-050 builds a credit scorecard. Cross-project matching finds:
- From bp-009 (zvt): Constraints about data quality validation (binary target checks, NaN handling) — high stage overlap with data_preparation
- From bp-002 (stock selection): Constraints about feature engineering pitfalls — medium stage overlap with feature_engineering
- From bp-020 (risk model): Constraints about model validation rigor — high kind transferability (domain_rule)

Excluded: bp-009 constraints about MACD parameters — zero stage overlap, zero when-clause similarity.
...
2. **所有知识内联**——不引用外部文件，不要求 AI read 外部文件
3. **路径参数化**（`{workspace}`）
...
| Human Summary | 在文件开头，用户可读，使用用户 locale 语言 |
| 路径参数化 | 无硬编码用户路径 |
...
| D35 | 约束覆盖率机械验证 | 运行 `scripts/crystal_quality_gate.py` 得到 `coverage.constraint_all.rate = 1.0`（seed.md 中唯一 finance-C-{n} ID 引用数 = 约束池总数） | 补齐缺失约束；禁止用摘要替代 ID 引用 |
