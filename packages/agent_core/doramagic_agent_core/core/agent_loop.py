...
from doramagic_agent_core.core.circuit_breaker import CircuitBreaker
...
from doramagic_agent_core.core.message import AgentMessage, ToolResult
...
class ExtractionAgent:
    ...
    def _save_trace(
        ...
        *,
        ...
        with contextlib.suppress(Exception):  # Trace saving is best-effort, never block pipeline
    ...
    async def run_structured_call(
        ...
        *,
        ...
        if l1_partial_text:
            try:
                from doramagic_agent_core.sop.executor import _extract_json

                extracted = _extract_json(l1_partial_text, require_type=dict)
                if extracted is not None:
                    validated = response_model.model_validate(extracted)
                    logger.info(
                        "run_structured_call L1.5 success: salvaged %s"
                        " from L1 completion (%d tokens)",
                        response_model.__name__,
                        total_tokens,
                    )
        ...
        if fallback_text:
            ...
            from doramagic_agent_core.sop.executor import _extract_json as _l3_extract
