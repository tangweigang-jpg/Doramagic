...
async def run_constraint_synthesis(
    ...
    if isinstance(result, RawFallback):
        ...
        if extracted is not None:
            ...
            logger.info(
                "constraint_synthesis: manual JSON extraction succeeded — "
                "%d kind changes applied",
                rebalanced_count,
            )
        else:
            logger.error(
        ...
        logger.info(
            "constraint_synthesis: Instructor L1/L2 success — "
            "%d kind changes applied (%d tokens)",
            rebalanced_count,
            total_tokens,
        )
            ...
            if old_kind != item.constraint_kind:
                ...
                if kind_counts.get(old_kind, 0) <= _MIN_KIND_RESERVE:
                    logger.info(
                        ...
                        idx, old_kind, item.constraint_kind,
                        old_kind, _MIN_KIND_RESERVE,
                    )
                    continue
                merged[idx]["constraint_kind"] = item.constraint_kind
                kind_counts[old_kind] -= 1
                kind_counts[item.constraint_kind] = (
                    kind_counts.get(item.constraint_kind, 0) + 1
                )
                changes += 1
                logger.debug(
...
def _build_synthesis_summary(merged: list[dict], max_items: int = 200) -> str:
    ...
    """
    lines: list[str] = [
        f"以下是需要审查的约束条目（仅包含 operational_lesson / claim_boundary）：\n"
    ]
    included = 0
    for idx, constraint in enumerate(merged):
        ...
        if included >= max_items:
            remaining = sum(
                1
                for c in merged[idx:]
                if c.get("constraint_kind", "") in _REVIEW_KINDS
            )
