...
□ constraint_kind 是否属于：domain_rule / resource_boundary /
...
□ source_type 是否属于：code_analysis / document_extraction /
...
### 6. rationalization_guard — 反合理化守卫
反合理化守卫记录 LLM 可能用来跳过规则的借口及其反驳。典型例子：
- "问题看起来简单" → 不能跳过根因调查
- "时间紧迫" → 不能跳过测试验证步骤
- "已经有假设了" → 不能跳过数据验证

搜索方向：SKILL.md 的 "Common Rationalizations" / "Red Flags" 段落、
CONTRIBUTING.md 中的行为禁令、源码注释中的 "DO NOT TOUCH/REMOVE/CHANGE" 模式、
❌/✅ 对比表。
注意：此类约束必须包含 guard_pattern 字段（excuse/rebuttal/red_flags/violation_detector）。
source_type 通常为 document_extraction。仅当项目含文档知识源时提取。"""
...
# ---------------------------------------------------------------------------
# v3 新增: 文档约束提取系统提示词 (SOP Step 2.1-s)
# ---------------------------------------------------------------------------

CON_DOC_EXTRACT_SYSTEM = (
    """\
你是约束提取专家。请从文档知识源中为蓝图提取约束。
...
|------|-----------|-----------------|
| 文档声明 + 代码中有对应实现 | code_analysis | 0.9 |
| 文档显式声明，未验证代码 | document_extraction | 0.6 |
| 从多个文档段落推断 | expert_reasoning | 0.5 |

## Evidence 格式

文档约束的 evidence_summary 格式为: `文件路径:§段落标题`
例如: `SKILL.md:§Phase-1-Step-4` 或 `CLAUDE.md:§Rules`

## 工具使用流程

1. 使用 read_file 逐个读取文档知识源
2. 按上述 5 种文档特征模式逐一扫描
3. 为每个发现生成一条约束
4. 使用 write_artifact 写入结果 JSON 到 ct_doc_constraints.json
...
- 禁止在 action 中使用模糊词
- 禁止发明枚举清单之外的值
- 禁止将示例(example)当作规则(normative)——注意区分
"""
)

# ---------------------------------------------------------------------------
# v3 新增: Rationalization Guard 提取系统提示词 (SOP Step 2.6)
# ---------------------------------------------------------------------------
...
## 什么是 Rationalization Guard

反合理化守卫记录 LLM 在执行技能/规则时可能使用的**借口**及其**反驳**。
这是一种独特的知识维度：代码不会自我辩解跳过执行，但 LLM 会。
...
- ❌ 行为 = 1 条约束
- "DO NOT" 注释 = 1 条约束（代码知识源）

## 输出格式

每条约束 constraint_kind 固定为 rationalization_guard，且必须包含 guard_pattern 字段：

```json
{
  "when": "agent 可能出现该合理化的场景",
  "modality": "must_not",
  "action": "简洁的禁止声明（如'禁止以问题太简单为由跳过根因调查'）",
  "consequence_kind": "根据跳步破坏的内容判断",
  "consequence_description": "具体描述跳步后的失败现象",
  "constraint_kind": "rationalization_guard",
  "severity": "破坏 correctness/compliance = fatal，降低质量 = high，影响效率 = medium",
...
1. 使用 read_file 读取文档知识源
2. 搜索上述扫描模式
3. 如果找不到相关内容，立即 write_artifact 写空 JSON: {"constraints": []}
4. 否则，为每个发现生成约束，使用 write_artifact 写入
...
- 禁止编造不存在于文档中的借口
- 所有约束的 constraint_kind 必须是 rationalization_guard
- guard_pattern 的 excuse 和 rebuttal 必须直接引用或准确转述文档原文
"""
)

# ---------------------------------------------------------------------------
# v3 新增: 独立约束评估系统提示词 (Sprint Contract)
# ---------------------------------------------------------------------------

CON_EVALUATOR_SYSTEM = """\
你是约束质量独立评估者。你没有参与约束提取过程。
你的任务是独立验证约束的质量，不能信任提取过程的任何声明。

## 4 项验证契约

### Contract 1 — Evidence Validity（证据有效性）
读取 evidence_summary 引用的文件和行号。
- 文件存在？使用 read_file 验证
- 行号在范围内？代码逻辑是否支撑约束声明？
- 判定：VALID | INVALID | WEAK

### Contract 2 — Kind Correctness（类型正确性）
反事实测试：如果将此约束归为其他 kind，是否更合适？
- domain_rule vs architecture_guardrail: 前者是业务规则，后者是实现约束
- operational_lesson vs claim_boundary: 前者是经验教训，后者是能力边界
- 判定：CORRECT | MISCLASSIFIED

### Contract 3 — Severity Calibration（严重度校准）
consequence_description 描述的后果是否匹配 severity 级别？
- fatal: 数据损坏/监管违规/资金损失
- high: 显著结果偏差
- medium: 性能/效率影响
- low: 编码风格/最佳实践
- 判定：CALIBRATED | OVER_SEVERITY | UNDER_SEVERITY

### Contract 4 — Triad Completeness（三元组完整性）
when + modality + action + consequence 是否完整且非循环？
- when 是否足够具体？
- action 是否可执行（含具体操作、字段名、阈值）？
- consequence 是否描述具体失败现象？
- 判定：COMPLETE | INCOMPLETE_WHEN | VAGUE_ACTION | WEAK_CONSEQUENCE
...
- PASS:         score >= 0.80 且无 INVALID evidence
- FIXABLE:      score >= 0.60 且所有 issue 有 fix_suggestion
- NEEDS_REWORK: score < 0.60 或存在结构性问题

## 禁止事项

- 禁止信任提取过程的声明——你必须独立验证每条证据
- 禁止跳过 evidence 验证（Contract 1 是最重要的契约）
- 禁止给出没有 fix_suggestion 的 issue
"""
