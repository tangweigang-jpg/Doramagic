#!/usr/bin/env bash
# ============================================================
# v10 第三波蓝图提取 — 停电中断恢复版（bp-097 起）
# 原批次 v10-wave3-20260417-090510 在 bp-097 (OpenBB) 的
# local_synthesis 阶段因停电中断，rotki/hummingbot 已完成。
# ============================================================
...
TOTAL=${#QUEUE[@]}
LOG_DIR="_runs/_batch/v10-wave3-resume-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$LOG_DIR"

SUMMARY_FILE="$LOG_DIR/summary.txt"
echo "v10 Wave 3 Resume — $(date)" > "$SUMMARY_FILE"
echo "Total: $TOTAL projects (resumed from bp-097 after power outage)" >> "$SUMMARY_FILE"
echo "========================================" >> "$SUMMARY_FILE"
...
echo ""
echo "============================================================"
echo "Wave 3 Resume Complete: $PASSED passed, $FAILED failed / $TOTAL"
echo "============================================================"
echo "" >> "$SUMMARY_FILE"
